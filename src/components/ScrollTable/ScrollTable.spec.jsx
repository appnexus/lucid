...
describe('ScrollTable', () => {
	...
	describe('render', () => {
		it('should render a container with a Table inside', () => {
			...
			assert(wrapper.is('.lucid-ScrollTable'), 'root must be a container element');
			assert(wrapper.not(Table), 'root must not be a Table');
			assert.equal(wrapper.find(Table).length, 1, 'must contain a Table component');
		});
	});
	...
	describe('children', () => {
		it('should match the child elements of the Table component', () => {
			assert.equal(Thead, Table.Thead, 'Thead child component must match Table.Thead');
			assert.equal(Tbody, Table.Tbody, 'Thead child component must match Table.Tbody');
