...
function indexAst() {
	const code = fs
		.readFileSync(path.join(CONFIG.SOURCE_DIR, 'index.js'))
		.toString();
...
function imports() {
	...
	return _.flatMap(importDeclarations, node => {
		...
		const newPath = isPath
			? './' + path.join(CONFIG.BUILD_DIR, moduleId)
			: null;
...
module.exports = {
	...
	jsPathImports: function() {
		...
		const checkFilesPromise = Promise.all(
			_.map(allExports, exportName => {
				return new Promise((resolve, reject) => {
					fs.stat(`${exportName}.js`, err => {
						if (!err) {
							return reject(
								new Error(
									`One of the exports (${exportName}) in index.js conflicts with a file you have on your file system (${exportName}.js). Unable to write path-import files.`
								)
							);
						}
						return resolve();
